...
- **디버그 탭.** 디버그 세션을 시작하면 디버그 툴윈도우에 **"로그 뷰어" 탭**이 자동으로 추가됩니다 —
  세션의 콘솔에 찍히는 모든 것(stdout/stderr는 물론, Rider .NET의 **`Debug.WriteLine` "Debug Output" 탭과
  디버거 메시지까지**)을 이 뷰어의 필터/하이라이트/에러 요약/vim으로 봅니다. 콘솔 뷰 자체를 구독하는
  방식(`ObservableConsoleView`)이라 설정이 전혀 없고, 늦게 생기는 콘솔 탭도 자동으로 잡습니다. 소스가
  둘 이상이면(콘솔 + Debug Output) 그리드 위에 **"출력 소스" 콤보**가 나타나 모두/개별 소스를 골라 볼
  수 있습니다 — 같은 줄을 stdout과 `Debug.WriteLine` 양쪽에 보내는 코드는 합친 보기에서 두 번 보이는 게
  정상이므로, 그때 한쪽만 고르면 됩니다. Follow 기본 켜짐, 세션 종료 후에도 콘솔처럼 남습니다. 줄
  형식/인코딩 기억은 실행 설정 이름 단위.
...
- **리더**(`LogReader`) — `LocalLogReader`(폴링 바이트 커서: 마지막 오프셋에서 재개, 부분 줄 버퍼링, 회전/절단
  ...
  `ByteLineSplitter`가 바이트→줄 분리를 담당합니다. 세 번째 소스는 `debug/ProcessOutputBuffer` —
  이미 디코딩된 텍스트 청크를 스트림별로 줄 조립해 세션 전체 히스토리를 보관(상한 100만 줄)합니다.
  `debug/DebugLogTabListener`(XDebuggerManagerListener)가 세션마다 이 버퍼를 물린 패널을 디버그 탭
  (RunnerLayoutUi Content)으로 설치하고, 세션의 **콘솔 뷰들을 구독**해(`ObservableConsoleView.addChangeListener`
  + `getText()` 시딩, 둘 다 EDT라 무손실·무중복) 버퍼를 채웁니다 — Rider .NET은 stdout·Debug.WriteLine·디버거
  메시지를 ProcessHandler가 아니라 콘솔에 직접 print하기 때문입니다. 늦게 생기는 콘솔 탭(Debug Output)은
  ContentManagerListener로 잡고, 관찰 가능한 콘솔이 하나도 없는 세션만 ProcessHandler 탭으로 폴백합니다.
