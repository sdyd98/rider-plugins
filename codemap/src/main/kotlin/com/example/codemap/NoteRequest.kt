...
object NoteRequest {
    ...
    private fun functionPrompt(relPath: String, symbol: String, question: String, existing: JsonObject?): String =
        buildString {
            ...
            appendLine()
            appendLine("규칙:")
            appendLine("- anchor 는 시그니처를 원문 그대로. 여는 중괄호는 빼고, 여러 줄이면 한 줄로 이어 적어라.")
    ...
    private fun notePrompt(
        ...
        appendLine("- anchor 는 시그니처를 원문 그대로 복사해라. 이걸로 위치를 찾으므로 파일에 실제로 있어야 한다.")
        appendLine("  여는 중괄호는 넣지 마라 (다음 줄에 있는 코드가 많다). 시그니처가 여러 줄이면 한 줄로 이어 적어라 —")
        appendLine("  공백·탭·줄바꿈은 무시하고 찾으므로 그래도 맞는다.")
