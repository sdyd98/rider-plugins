...
`testdata/cpp` 는 **틀리기 쉬운 것만 모아둔** C++ 솔루션입니다. 각 줄에 "함수여야 한다 / 아니어야 한다" 표식이 붙어 있고, 스크립트가 샌드박스를 띄워 백엔드 응답과 대조합니다. 어긋나면 줄 번호와 원문을 찍고 0이 아닌 코드로 끝납니다.

덮는 것: 오버로드, 여러 줄 시그니처, 매크로로 선언·정의된 함수, 템플릿 함수와 클래스 템플릿, 다중 상속과 `override`, 중첩 클래스, 연산자 오버로드, 생성자·소멸자·`= default`·`= delete`, `inline`/`constexpr`/`static`, 가변 인자, most vexing parse. 그리고 **함수처럼 보이지만 아닌 것들** — 함수 포인터 typedef, 람다 변수, 멤버 변수, `extern` 선언.

현재 **61개 전부 일치, 오탐 0**입니다.

이 대조가 실제로 세 가지를 잡았습니다:
- 람다 변수(`static auto f = [](const Packet& p) {…}`)를 함수로 셌습니다 — 람다의 매개변수 목록이 변수 선언에 속해서입니다
- `= default` 를 선언으로 취급했습니다. 생성자의 정의를 찾는 사람은 거기서 찾은 것이니 정의가 맞습니다(`= delete` 는 아닙니다)
- 픽스처 자신의 주석이 표식으로 오인됐습니다 — 검증 장치도 검증이 필요합니다

## 아직 안 되는 것

- **진짜 코드베이스에는 안 대봤습니다.** 픽스처는 어려운 케이스를 덮지만 5800만 줄의 규모·빌드 설정·전처리기 지옥은 다른 문제입니다
- 시그니처는 선언문 첫 줄입니다. 여러 줄 시그니처는 첫 줄만 남습니다(위치는 정확합니다)
